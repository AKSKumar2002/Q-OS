...
import { NAVIGATION_STRUCTURE, ModuleId } from "../types";
...
export function Layout({ children, activeModule, setActiveModule, onLogout, user: currentUser }: LayoutProps) {
    ...
    <div className="flex h-screen overflow-hidden p-4 gap-4">
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 custom-scrollbar">
          {NAVIGATION_STRUCTURE.map((group, idx) => (
            <div key={idx} className="mb-6">
              ...
              <div className="space-y-0.5">
                {group.items.filter(item => {
                  ...
                  if (item.id === "crm-job-cards") {
                    return tenant.installedApps.includes("crm") && tenant.features.includes("job_cards");
                  }
                  return true;
                }).map((item) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (item.id === "crm-job-cards" && user.role === "Viewer") {
                        alert("Access Denied: Read-Only Role");
                        return;
                      }
