...
// --- Prescription Grid ---
export const PrescriptionGrid = ({ data }: { data?: any }) => (
  <div className="border border-slate-200 rounded-xl overflow-hidden glass-card">
    <table className="w-full text-sm text-center">
      <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-200">
        <tr>
          <th className="p-2 border-r border-slate-200 text-xs font-bold text-slate-500 uppercase">Eye</th>
          <th className="p-2 border-r border-slate-200 text-xs font-bold text-slate-500 uppercase">Sph</th>
          <th className="p-2 border-r border-slate-200 text-xs font-bold text-slate-500 uppercase">Cyl</th>
          <th className="p-2 border-r border-slate-200 text-xs font-bold text-slate-500 uppercase">Axis</th>
          <th className="p-2 border-r border-slate-200 text-xs font-bold text-slate-500 uppercase">Add</th>
          <th className="p-2 text-xs font-bold text-slate-500 uppercase">PD</th>
        </tr>
      </thead>
      <tbody>
        <tr className="border-b border-slate-100 group hover:bg-white/40">
          <td className="p-2 border-r border-slate-200 font-bold text-slate-600 bg-slate-50/30">OD (Right)</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.od?.sph || "0.00"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.od?.cyl || "0.00"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.od?.axis || "0"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.od?.add || "0.00"}</td>
          <td className="p-2 row-span-2 align-middle font-mono font-bold text-slate-800 border-l border-slate-200 bg-slate-50/10">
            {data?.pd || "62"}
          </td>
        </tr>
        <tr className="group hover:bg-white/40">
          <td className="p-2 border-r border-slate-200 font-bold text-slate-600 bg-slate-50/30">OS (Left)</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.os?.sph || "0.00"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.os?.cyl || "0.00"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.os?.axis || "0"}</td>
          <td className="p-2 border-r border-slate-200 font-mono font-medium">{data?.os?.add || "0.00"}</td>
        </tr>
      </tbody>
    </table>
  </div>
